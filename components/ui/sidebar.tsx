...
const sidebarVariants = cva(
  "fixed inset-y-0 left-0 z-50 flex h-full flex-col border-r bg-sidebar shadow-sm transition-transform duration-300 ease-in-out dark:border-slate-700",
  {
    variants: {
      variant: {
        default: "w-64",
        floating: "m-2 h-[calc(100%-1rem)] rounded-xl border",
      },
    },
    defaultVariants: {
      variant: "default",
...
export function SidebarTrigger() {
  ...
  return (
    <button
      type="button"
      className="inline-flex h-10 w-10 items-center justify-center rounded-md text-slate-500 hover:text-slate-900 md:hidden"
...
export function SidebarInset({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn("flex-1 md:ml-64", className)} {...props}>
      {children}
    </div>
  )
}
...
export function SidebarGroupLabel({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn("mb-2 text-xs font-semibold uppercase text-slate-500", className)} {...props} />
}
